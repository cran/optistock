...
rbtr_growth_params <-
  growth_parameters %>%
  filter(spp == "rainbow_trout") %>%
  select(linf, k, t0) %>%
  as.list()
rbtr_cost_params <-
  cost_parameters %>%
  filter(spp == "rainbow_trout", cost_fun == "linear_total_cost")


linf <- rbtr_growth_params$linf
k <- rbtr_growth_params$k
t0 <- rbtr_growth_params$t0
rec_length <- 20.3
...
exp_args <- list(m_init = 0.02, m_inf = 0.0001, alpha = 0.05)
...
bottleneck_mort_data <- tibble(
  ...
  mort_params = list(
    c(list(m_init = 0.0025, m_max = 0.02), bottleneck_args),
    c(list(m_init = 0.0015, m_max = 0.01), bottleneck_args),
    c(list(m_init = 0.001, m_max = 0.005), bottleneck_args)
  )
)
